...
function buildGridItemForPanel(panel: PanelKind): DashboardGridItem {
  ...
  const vizPanelState: VizPanelState = {
    ...
    pluginId: panel.spec.kind ?? 'timeseries',
    options: panel.spec.options ?? {},
    fieldConfig: panel.spec.fieldConfig,
  ...
  if (panel.spec.timeFrom || panel.spec.timeShift) {
    vizPanelState.$timeRange = new PanelTimeRange({
      timeFrom: panel.spec.timeFrom,
      timeShift: panel.spec.timeShift,
      hideTimeOverride: panel.spec.hideTimeOverride,
    });
  }
